...
then
    ...
    then
        CMD="$CMD -l $DATA_FILE_DETAILED -r"
    else
        CMD="$CMD -l $DATA_FILE_DETAILED"
    fi
elif [[ $REUSE_FILE != "" ]];
then
    DATA_FOLD_TIME=`dirname($REUSE_FILE)`
    INFO_FILE=`find . -type f -iname "Info*"`
    part=0
    while [ -e $REUSE_FILE${part}.org ]; do 
        bkup=`expr $part + 1`
    done
    cp $REUSE_FILE $REUSE_FILE${part}
