...
* TODO [0/2]
  - [ ] Run brute force with load_overlap = [true,false],
    ...
    local_work_size
  - [ ] Try D-Optimal design if we can get enough information
* Memo
  - Get opencl platform
...
** 2016-03-16
*** Optimal design 
    - Building D-Optimal design
      #+begin_src R :results output graphics :file (org-babel-temp-file "figure" ".png") :exports both :width 600 :height 400 :session
        library(DoE.base)
        library(DoE.wrapper)
        Design.1 <- lhs.design( type= "maximin" , nruns= 200 ,nfactors= 5 ,digits= NULL ,seed=20049 , factor.names=list(log_x_component_number=c(1,4), y_component_number=c(1,4),
                                                                                                                        log_vector_length=c(1,4), local_work_size_index=c(1,10),
        ...
        Design.1.Dopt <- Dopt.design( 40 , data= Design.1.rounded , formula= 
      ...
      #+RESULTS:
      [[file:/tmp/babel-4662-nm/figure4662CNq.png]]

    - Applying the D-optimal design
     ...
     - With our generated experiment planning we extract the
       corresponding values from the experiment measurements and
       check if we have enough informations
        #+begin_src R :results output :session :exports both
          df <- read.csv("/tmp/test.csv",strip.white=T,header=T)
          attach(df)
          ...
          for(i in 1:nrow(Design.1.Dopt)){
              set <- rbind(set, df[ x_component_number == 2^Design.1.Dopt$log_x_component_number[i]
                                   & y_component_number == Design.1.Dopt$y_component_number[i]
                                   & vector_length == 2^Design.1.Dopt$log_vector_length[i]
                                   & temporary_size == 2^Design.1.Dopt$log_temporary_size[i] 
                                   & as.numeric(local_work_size) == Design.1.Dopt$local_work_size_index[i], ])
        ...
        Residuals:
               Min         1Q     Median         3Q        Max 
        -2.158e-10 -1.168e-10 -2.070e-11  5.365e-11  4.135e-10 

        Coefficients:
                             Estimate Std. Error t value Pr(>|t|)    
        (Intercept)         3.911e-10  6.803e-11   5.748 1.51e-06 ***
        x_component_number  8.025e-13  4.031e-12   0.199 0.843301    
        y_component_number -6.630e-11  1.810e-11  -3.662 0.000797 ***
        vector_length       2.632e-11  3.947e-12   6.668 8.97e-08 ***
        ---
        Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

        Residual standard error: 1.51e-10 on 36 degrees of freedom
        Multiple R-squared:  0.6021,	Adjusted R-squared:  0.5689 
        F-statistic: 18.16 on 3 and 36 DF,  p-value: 2.412e-07
#+end_example

    - How does it compare to a random sample?
        Some times it can find the impact of the x_component_number but
        it is rare and generally the R-squared is worse
        #+begin_src R :results output :session :exports both
          df <- read.csv("/tmp/test.csv",strip.white=T,header=T)
          random_sample <- df[sample(1:length(df$x_component_number),size = 80,replace = FALSE),]
        ...
        Residuals:
               Min         1Q     Median         3Q        Max 
        -2.910e-10 -1.257e-10 -3.724e-11  7.628e-11  6.801e-10 

        Coefficients:
                             Estimate Std. Error t value Pr(>|t|)    
        (Intercept)         4.821e-10  6.474e-11   7.446 1.26e-10 ***
        x_component_number -1.271e-12  3.893e-12  -0.327    0.745    
        y_component_number -1.041e-10  1.939e-11  -5.367 8.38e-07 ***
        vector_length       3.436e-11  4.250e-12   8.083 7.62e-12 ***
        ---
        Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

        Residual standard error: 1.824e-10 on 76 degrees of freedom
        Multiple R-squared:  0.5616,	Adjusted R-squared:  0.5443 
        F-statistic: 32.46 on 3 and 76 DF,  p-value: 1.306e-13
        #+end_example
    ...
    - How does it compare to LHS?
        #+begin_src R :results output :session :exports both
          ...
          Design.2 <- lhs.design( type= "maximin" , nruns= 80 ,nfactors= 5 ,digits= NULL ,seed=20049 , factor.names=list(log_x_component_number=c(1,4), y_component_number=c(1,4),
                                                                                                                         log_vector_length=c(1,4), local_work_size_index=c(1,10),
                                                                                                                         log_temporary_size=c(1,2) ) )
        ...
        Residuals:
               Min         1Q     Median         3Q        Max 
        -1.327e-10 -8.527e-11 -2.187e-11  4.876e-11  5.156e-10 

        Coefficients:
                             Estimate Std. Error t value Pr(>|t|)    
        (Intercept)         4.166e-10  4.582e-11   9.092 8.90e-14 ***
        x_component_number -8.715e-13  3.047e-12  -0.286    0.776    
        y_component_number -8.671e-11  1.420e-11  -6.108 3.98e-08 ***
        vector_length       1.983e-11  2.956e-12   6.710 3.08e-09 ***
        ---
        Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

        Residual standard error: 1.219e-10 on 76 degrees of freedom
        Multiple R-squared:  0.5237,	Adjusted R-squared:  0.5049 
        F-statistic: 27.85 on 3 and 76 DF,  p-value: 2.956e-12
        #+end_example
