...
*** Optimal design 
    #+begin_src R :results output graphics :file (org-babel-temp-file "figure" ".png") :exports both :width 600 :height 400 :session
      library(DoE.base)
      library(DoE.wrapper)
      Design.1 <- lhs.design( type= "maximin" , nruns= 200 ,nfactors= 7 ,digits= NULL ,seed=20049 , factor.names=list( log_x_component_number=c(1,4),y_component_number=c(1,4),
                                                                                                                      log_vector_length=c(1,4),vector_recompute=c(0,1),load_overlap=c(0,1),
                                                                                                                      local_work_size_index=c(1,10),log_temporary_size=c(1,2) ) )
      ...
      plot( Design.1 , select = c( "log_x_component_number","y_component_number",
                                  "log_vector_length","vector_recompute","load_overlap","local_work_size_index",
                                  "log_temporary_size" ))
    #+end_src

    #+RESULTS:
    [[file:/tmp/babel-8208SmU/figure8208x1H.png]]
    - Should we round the lhs values?
      #+begin_src R :results output graphics :file (org-babel-temp-file "figure" ".png") :exports both :width 600 :height 400 :session
        Design.1.rounded <- round(Design.1)
  
        plot( Design.1.rounded , select = c( "log_x_component_number","y_component_number",
                                    "log_vector_length","vector_recompute","load_overlap","local_work_size_index",
                                    "log_temporary_size" ))
      #+end_src

      #+RESULTS:
      [[file:/tmp/babel-8208SmU/figure8208leg.png]]
      #+begin_src R :results output graphics :file (org-babel-temp-file "figure" ".png") :exports both :width 600 :height 400 :session
        Design.1.Dopt <- Dopt.design( 40 , data= Design.1.rounded , formula= 
                                                              "~(log_x_component_number  + y_component_number  + log_vector_length  + vector_recompute  + load_overlap  + local_work_size_index  + log_temporary_size )^2" , nRepeat= 5 ,randomize= TRUE ,seed= 28517 )
        ## creator element of design.info will be different, when using the command line command!
        plot( Design.1.Dopt , select = c( "log_x_component_number","y_component_number","log_vector_length","vector_recompute","load_overlap","local_work_size_index","log_temporary_size" ))
      #+end_src

      #+RESULTS:
      [[file:/tmp/babel-8208SmU/figure8208yom.png]]
    - Or should we round the D-optimal values?
      #+begin_src R :results output graphics :file (org-babel-temp-file "figure" ".png") :exports both :width 600 :height 400 :session
        Design.1.Dopt2 <- Dopt.design( 40 , data= Design.1 , formula= 
                                                              "~(log_x_component_number  + y_component_number  + log_vector_length  + vector_recompute  + load_overlap  + local_work_size_index  + log_temporary_size )^2" , nRepeat= 5 ,randomize= TRUE ,seed= 28517 )
        ## creator element of design.info will be different, when using the command line command!
        Design.1.Dopt2 <- round(Design.1.Dopt2)
        plot( Design.1.Dopt2 , select = c( "log_x_component_number","y_component_number","log_vector_length","vector_recompute","load_overlap","local_work_size_index","log_temporary_size" ))
      #+end_src
      #+RESULTS:
      [[file:/tmp/babel-8208SmU/figure8208_ys.png]]
    - Even if we have rounded things we don't have duplicated combinations
      #+begin_src R :results output :session :exports both
        duplicated(Design.1)
      #+end_src
    ...
    - Mapping the local_worksize
     ...
     #+RESULTS:
     #+begin_example
      log_x_component_number vector_length  y_component_number temporary_size
      Min.   : 1.0       Min.   : 1.0   Min.   :1.00       Min.   :2     
      1st Qu.: 2.0       1st Qu.: 2.0   1st Qu.:1.75       1st Qu.:2     
      Median : 4.0       Median : 4.0   Median :2.50       Median :3     
      Mean   : 6.2       Mean   : 6.2   Mean   :2.50       Mean   :3     
      ...
      - Want to keep the point of the D-optimal but it seems it is not
        the correct way to day it...
      #+begin_src R :results output :session :exports both
               df[ x_component_number == 2^Design.1.Dopt$log_x_component_number & 
                   y_component_number == Design.1.Dopt$y_component_number & 
                   vector_length == 2^Design.1.Dopt$log_vector_length &
                   vector_recompute == as.logical(Design.1.Dopt$vector_recompute) &
                   load_overlap == as.logical(Design.1.Dopt$load_overlap) &
                   temporary_size == 2^Design.1.Dopt$log_temporary_size &
                   as.numeric(local_work_size) == Design.1.Dopt$local_work_size_index, ]


        # "x_component_number","y_component_number","log_vector_length","vector_recompute","load_overlap","local_work_size_index","log_temporary_size" 
      #+end_src

      #+RESULTS:
      : [1] x_component_number vector_length      y_component_number temporary_size    
      : [5] vector_recompute   load_overlap       local_work_size    time_per_pixel    
