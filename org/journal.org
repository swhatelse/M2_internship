...
**** General architecture
   ...
   - SMX: corresponds to OpenCL compute unit
     Compute a thread block. The register file is the set of registers
     that is shared by the threads in the SMX. In a SMX more than one
...
**** Programming a GPU
   - Thread block are split into warps (group of 32 threads) a nd
     schedule on SMX. SMX can execute mulitples block at once.
   - Notes: CUDA is scalar architecture. Vectorization does not brings
     anything?
   - Cuda <-> OpenCL
     - SMX = compute unit
     - CUDA thread = work item
     - Thread-block = work group
     - Grid = NDRange
   - Number of register for a thread-block = ceil(number of register
     for the kernel x ceil(Number of thread per block,32), Number of
     register max per SMX / 32 )
*** Changing the search space
