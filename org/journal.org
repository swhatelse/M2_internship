...
*** OUT_OF_RESSOURCES constraints
   - Config limit
     ...
     #+begin_src R :results output :session :exports both
       f <- function(elements_number, y_component_number=1, vector_length=1, temporary_size=2, threads_number=32) {
           x_component_number = (elements_number / y_component_number)
           vector_number = ceiling(x_component_number / vector_length)
           total_x_size = vector_number * vector_length

           tempload = (total_x_size / vector_length) * vector_length #+ total_x_size %/% vector_length
           temp = 3 * vector_number * (y_component_number+2) * vector_length
           res = vector_number * y_component_number * vector_length
           tempc = 3 * vector_number * (y_component_number+2) * vector_length * temporary_size
           resc = vector_number * y_component_number * vector_length * temporary_size
           tot <- (tempload + temp + res + tempc + resc) * threads_number
       }

       print(f(53,1,1,2,1024))
       print(f(6,3,16,2,1024))
     #+end_src
