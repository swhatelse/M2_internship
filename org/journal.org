...
**** Quick analysis of Laplacian.rb
     To be able to understand the result I must first understand
     exactly what is done.
     
     - {:x_component_number=>8, :vector_length=>2, :y_component_number=>1, :temporary_size=>4, :vector_recompute=>false, :load_overlap=>false}
       #+BEGIN_SRC C
...
*** Changing the search space
    - By exlporing values for global_work_size and local_work_size
      - Local_size \to limited by the physical device
      - Global_size \to should be a multiple of the local_work_size and
        depend of the size of the work. So it should be image_size /
        local_work_size. It specifies the number of work-groups in each
        dimension (1 \to 3)
