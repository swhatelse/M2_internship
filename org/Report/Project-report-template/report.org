...
* Analysis
  ...
  detailed study of the search space to understand our results. As we
  used the linear regression of expectation and the quantile
  regression we studied there difference. Then we studied the impact
  of the choice of the model and the sampling techniques on our
  approach. Finally we dig into optimization techniques to converge
...
**** 
    ...
    between regression of expectation and quantile regression. Using
    the regression of expectation estimates that the best version has
    a x_component_number of 4 which is not true. While using quantile
    regression of the 5th succeed in predict that the best version is
    a version with a x_component_number of 1. So the regression of
    expectation may not find the minimum while the quantile regression
    will if the model is correct.
    There are different ways of computing quantile regression we tried
    the rq function of R it gives good estimation of the coefficient
    of the different parameters but for some reason it fails with our
    data to compute the standard error and the R-squared. Instead we
    use another techniques that use performs weighted least square to
    apply more weight to the desired quantile. And it iterates in
    order to converge to the quantile regression of the wanted
    quantile. Nevertheless, we do not yet master this technique
    completely, we managed to get the correct coefficients but we do not
    know how to determine correctly the minimum number of iterations
    to converge and how to determine correct the bounds for the
    weights. The other difficult with this method is that we do not
    really know how to interpret the standard error, how to compute
    confidence intervals and the R-squared values are too
    optimistic. Hence it is a bit more complicated to make inferences
    about models. 
