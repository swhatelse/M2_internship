...
*** Model approximation
   ...
   has ideas about their potential effects. He not only needs to check his
...
****                                                               :noexport:
     ...
     #+begin_src R :results output graphics :file (org-babel-temp-file "figure" ".png") :exports both :width 600 :height 400 :session
       ...
       lines(elements_number_val,predict(model_elem_nb,data.frame(elements_number=elements_number_val),interval="none"),col="red")  

       # plot(y=df$time_per_pixel,df$y_component_number, ylim=c(0,5e-10))
       plot(y=df$time_per_pixel,df$y_component_number)
       lines(y_component_number_val,predict(model_y_comp,data.frame(y_component_number=y_component_number_val),interval="none"),col="red")  

       # plot(y=df$time_per_pixel,df$vector_length, ylim=c(0,5e-10))
       plot(y=df$time_per_pixel,df$vector_length)
       lines(vector_length_val,predict(model_v_len,data.frame(vector_length=vector_length_val),interval="none"),col="red")  

       # plot(y=df$time_per_pixel,df$threads_number, ylim=c(0,4e-10))
       plot(y=df$time_per_pixel,df$threads_number)
       lines(threads_number_val,predict(model_th_nb,data.frame(threads_number=threads_number_val),interval="none"),col="red")  

       # plot(y=df$time_per_pixel,df$lws_y, ylim=c(0,4e-9))
       plot(y=df$time_per_pixel,df$lws_y)
       lines(lws_y_val,predict(model_lws_y,data.frame(lws_y=lws_y_val)),col="red")  
     ...
     #+RESULTS:
     [[file:/tmp/babel-3768Xi7/figure3768UNk.png]]

**** 
    Our approach allows the expert to check his hypothesis to refine
    his model. The first step is
