...
*** Why is performance optimization difficult
   - In HPC code optimization crucial to exploit very complex hardware.
     ...
     - many cores \to heavy parallelism
     - pipelining ILP \to 
     - vector support \to SIMD
     - cache hierarchies
...
* Introduction
  ...
  are extremely complex machines. It not possible to wait for the next
  generation of hardware to bring automatically a speedup as it was
  the case at the beginning because the frequency doesn't increase
  anymore and in contrary tends to decrease. We went from
  multi-cores to many-cores architectures, for 2020 exascale
  platforms, with millions of cores, are expected in order to reach
  the exaflops. Thus, scientists have to take into account this
  massive parallelism when writting the code. Hardware can also be of
  type Single Instruction Multiple DATA (SIMD) and provide the
  support for vector operations and additional operations can be
  performs for free. 
  ...
  pipeline. Finally to add a little more complexity we also use GPUs
  require which are totally different from the CPUs. As a result performance
  optimization is difficult to achieve adding to that the fact that
  there are many HPCs platforms with different hardware which make
  optimization work well one supercomputer and bad on another
  one. Porting application is very time consumming and can also be
  very tricky.

  Today's machine also
  have different 
* Problem analysis
* State of the art
* Methods
