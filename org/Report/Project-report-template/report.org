...
* DONE Future work
  ...
  for the quantile regression to be able to access the quality of a
  model. We know that the quantile regression is better than the
  classical least square regression but if we cannot access if the
  ...
  deal with our constrained search space with a barrier approach is
  order to guide the optimization in feasible regions.  
...
* Conclusion
  ...
  code automatically. There are the compilers, the source-to-source
  transformation framework, and the meta-programming framework. They
  ...
  consider this approach has mathematical optimization problem as the
  search space can be huge and the exhaustive search is
  prohibitive. In our knowledge all auto-tuning are fully automatic
  process which exclude the user from the optimization phase of the
  process. Thus he cannot access the quality of the solution and
  cannot know if further improvements can be made. The main
  contribution of this report was to design an innovative approach
  which is semi-automatic and which involves the user in the
  optimization process, uses its knowledge and also guides him. Our
  approach relies on regression techniques to predict the objective
  ...
  method significantly outclassed them. The few cases where it did
  worst than a random search and a genetic algorithm came from the
  fact that for evaluation purpose we automated the process by using
  the same models and the same pruning choices without considering the
  working sample. By re-injecting the user is the optimization loop we
  showed that such case can be detected and handled correctly in order
  to improve the result. We also demonstrated that linear regression
  can have limitation for this kind of application because we have no
  ...
  this, quantile regression is more suited and improve the worst case
