...
** The reason why linear regression is not suited
   ...
   follow the same evolution as the mean. The resulting model and
   approximation can still be correct but this makes us doing
   assumptions that about the error and we do not know anything about
   the error as it is the case in the right in figure\ref{fig:lm-1}  
