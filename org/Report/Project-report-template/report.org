...
*** 
    ...
    39%. 
    The technique using least square regression has a higher rate of
    high quality solution than the genetic algorithm. 50% of the time
    the slowdown is less than 8%. However, sometimes it fails to find
    high performing versions and can be worst than GA with a maximum
    slowdown of x2.102. The quantile regression is the most effective
    techniques here with an even higher rate of high performing
    versions, half of the time it is very close the optimal solution
    with a slowdown that is lower than 1.2%. The worst solution is
    better than with LM with a maximum slowdown of x1.805 however that
    is still 29% higher than the worst solution of the GA.
   ...
   #+END_LaTeX
    ...
    | LM      | 1.010 |  2.102 | 1.165 |  1.080 |     118.8 |
    | RQ      | 1.010 |  1.805 | 1.094 |  1.012 |       119 |
...
* Analysis
  ...
  is not intended to use like this. Hence this part explains how
  results can be improved by using sampling strategy and choosing and
  refining the model according to the target problem. Finally we dig
  into constrained optimization techniques to converge faster to the
  optimal solution using our model of the objective function. 
...
*** Random
    ...
    which has a constraints and is discrete it provides a good covering.
*** Screening design
    The screening design was not suited in our case due to the
    constraints of the search space. It samples only the low and high
    value of the factors. Which means it only take points that are at
    the border of the search space. This gives a total of 128 possible
    combinations. However because of the constraints of the search
    space we only have 15 points left which is not enough with a
    search space of this size.
*** LHS
*** D-Optimal
