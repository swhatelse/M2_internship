...
* DONE Controlling measurement
   ...
   #+begin_src R :results output graphics :file img/warm_up.pdf :exports none :width 800 :height 600 :session
     library(plyr)
     library(ggplot2)
   ...
   \begin{figure}[]
   \centering
   \includegraphics[height=.15\textheight]{./img/warm_up.pdf}
   \caption{\label{fig:warmup}Warm-up effect}
   \end{figure}
   #+END_LaTeX
* TODO Results
***                                                                :noexport:
    #+begin_src R :results output graphics :file ./img/results_hist.pdf :exports none :width 600 :height 800 :session
...
*** 
    ...
    - LHS: it is not methods use to search, it is a sampling
      ...
      perform. 
    - GR: greedy search which is a derivative-free local search. From
      a starting point it explores all the possible directions at
      distance one and goes to the direction that gives the best
      improvement. This kind of algorithm is very efficient on convex
      objective function or if we already know where to search.  
    
    There are many ways of performing linear regression. We evaluated
    two of them: 
    - LM: which is the least square regression which gives an estimate
      of the mean.
    - RQ: which is the quantile regression which gives an estimate of
      a given quantile.

    We measured each methods one hundred time with about 120 (0.5% of
    the search space) points for GA, RS, LHS, LM and RQ and we
    evaluated the slowdown achieved compared to the best solution
    available in the entire search space. For GR 1 random point is
    chosen as starting point.

    Our approach uses the random uniform sampling techniques to sample
    the search space. It starts with 50 random points and adds just
    enough points after pruning to perform the regression. The total
    number of points used lays between 84 and 89
    additional points when needed to be able to perform the
    regression but we do not use more than 120 points. In order to
    automate our approach for evaluation purpose, we decided to apply
    exactly the same strategy. This means we fixed the parameters in
    the same order, thus pruning decisions are the same and we used
    exactly the same model of without considering the structure of the
    random set.

    Figure\ref{search_comparison} shows the  results. 
    With this search space, the local search GR is inefficient,
    half of the time we get a slowdown of higher than x2. It can be
    every far from the optimal solution, up to x66 slower.
    The LHS search does a lot more better with a slowdown that is
    never higher than 38.8%. 50% of the time we can get a slowdown of
    17%.
    ...
    maximum slowdown of slowdown.
    The genetic algorithm GA is the one that performs the best in
    general with a slowdown which is less than 7.3% half of the time
    and a maximum slowdown of 39%.
   #+BEGIN_LaTeX
   \begin{figure}[]
   \centering
   \includegraphics[height=.15\textheight]{./img/results_hist.pdf}
...
* Analysis
  This part explains the results of our approach. First we made a
  detailed study of the search to understand our results. As we used the
  linear regression of expectation and the quantile regression we
  studied there difference. Then we studied the impact of the choice
  of the model and the sampling techniques on our approach. Finally we
  dig into optimization techniques to converge faster to the optimal
  solution using our model of the objective function.
...
*** The choice of quantile regression
   ...
   [[file:img/why_we_choose_quantile_reg.png]]
    ...
    need to estimate the minimum and regression on the 5th percentile is
    a generally a good estimation of
    it\cite{books/daglib/0076234}. There are different ways of
    computing quantile regression we just tried one which is the
    iterated weighted least squares. This techniques weighted least
    square regression to add more weight to a particular quantile and
    iterate until it has converge to the wanted quantile
    regression. Figure\ref{fig:qr-example} illustrates the comparison
    between regression of expectation and quantile regression.
