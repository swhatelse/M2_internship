...
*** 
    ...
    - LHS: it is not methods use to search, it is sampling which take
      point randomly but which also maximize the distance between the
      point to cover the full search space. However we want to see how
      a search based in it would perform.
    - GR: greedy search which is a local search. From a starting point
      it explores all the possible directions it can go at distance
      one and goes to the direction that gives the best improvement
      this kind of algorithm is very efficient on convex search space
      or if we already know where to search. 
    
    There are many ways of performing linear regression. With our
    approach, we evaluated two of them:
    ...
    We measured each methods one hundred time with about 120
    points for GA, RS, LHS, LM and RQ and we evaluated the slowdown
    achieved compared to the best solution available in the entire
    search space. Figure\ref{search_comparison} shows the
    results. With this search space, the local search is inefficient,
    half of the time we get a slowdown of higher than x2. The LHS
    search does better with a slowdown that is below 1.388
   #+BEGIN_LaTeX
   \begin{figure}[]
   \centering
   ...
   #+END_LaTeX
    ...
    |---------+-------+--------+-------+--------|
    | GR      | 1.006 | 66.010 | 7.964 |  2.161 |
    | LHS     | 1.009 |  1.388 | 1.155 |  1.172 |
    | RS      | 1.011 |  1.341 | 1.098 |  1.079 |
    | GA      |  1.00 |  1.390 | 1.098 |  1.073 |
    | LM      | 1.010 | 32.060 | 1.445 |  1.080 |
    | RQ      | 1.010 | 30.890 | 1.432 |  1.073 |
