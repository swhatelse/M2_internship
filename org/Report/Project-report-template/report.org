...
** Optimizing: the auto-tuning approach
  ...
  are the compiler flags, source to transformation and high-level
  modifications (e.g. the size of the a tile, block or
  vector). The auto-tuning consider all this a huge optimization
  problem and try to find the best combination of parameters.
