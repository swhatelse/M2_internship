...
** Code generation and opportunities  
  ...
  compiler to be effective. That is why frameworks such Orio\cite{} for source-to-source
  transformation have been developed. This approach generally use
  annotations to describe the optimizations. It allows to bring user's
  knowledge in the process of generation of an optimized code. The
  drawbacks are that the it is restricted to one language 
  ...
  The meta-programming approach goes further by giving more
...
** Optimizing: the auto-tuning approach
  ...
  are the compiler flags, source-to-source transformation and high-level
  modifications (e.g. the size of the a tile, block or
  vector). The auto-tuning consider all this as a huge optimization
  problem and try to find the best combination of parameters. The
  search space can be huge, and the exhaustive is prohibitive. Hence
  many techniques
