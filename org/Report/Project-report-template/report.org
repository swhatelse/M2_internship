...
* DONE Envisioned general approach
   ...
   the optimization process and not a lot of control. How good is the
   optimized version of the code? How is it possible to improve it?
   What does the search space look like? What are the parameters that
   have a big impact (high-order parameters) and those which have a
   small impact (low-order parameters)? All this questions are
   necessary to understand the structure of the problem. Our approach
   targets advanced users who need answers to this kind of questions
   to help them to understand the characteristics of the search space,
   to be able to prune it correctly and to choose the most suited
   ...
   build a model in order to approximate it at low cost. Our approach
   use linear regression because usually, it is enough to model
   ...
   2. The search space is sampled by taking into account the objective
      ...
      overview of the high-order parameters or if to refine his model or
      if he wants obtain more information about a precise part of the
      search space.
   3. Linear regression build the model based on the hypothesis
      provided by the user and also determines what are the parameters
   ...
   4. According to the model the best value for the studied parameters
      are predicted.
   5. Results of the regression and estimated best value for the
      parameters are return to the user. At this step either he is
      satisfied by the results of the regression and can prune the
   ...
   process where the user refine is model according the information he
   has. 
...
*** 
   ...
   \caption{\label{fig:1}Search comparison}
   \end{figure}
   #+END_LaTeX
* Analysis
