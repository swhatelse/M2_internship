...
*** Explanation of LM success and "failures"
    - Failure :
      Happened 2 times
...
*** 
    ...
    worst and the best solution to understand why some time it
    ...
    First, the *p-values* give information about the importance of a
    ...
    fix a factors that has a small impact to early would lead to a
    premature pruning of the search and as a results lots of good
    combinations would be missed.

    Then *coefficient of* *determination* tells about the fit of a model,
    the closer to 1 it is the better the model explain the variance. 
    However when comparing model this is not sufficient, and the order
    of magnitude of the standard error compared to estimate of the
    coefficients has to be taken into consideration. This gives
    information about the uncertainty in the estimate of the
    coefficient, the bigger is the *standard error* compared to the
    coefficient the more inaccurate the prediction. It is possible to
    have an acceptable R-squared but a too high standard deviation
    this will gives a bad prediction.  
    ...
    -2.316e-11, the standard error is very big compared to the
    ...
    length should be the smallest or the biggest possible. Another way
    of accessing the quality of a model is to compare the predicted
    values and the real ones, the more they correlated the more
    accurate the model. The figure\ref{fig:bad_prediction} 
    This kind
    of cases can be detected easily by the user and thus take correct
    decision. The explanation of why this worst case happened is because
    our automated experiment process did not make the previous analysis and
    applied the same strategy whether the model is correct or not.   
  ...
  #+NAME:   tab:correct-fit
    ...
    |---------------+-----------+-----------+----------|
    | /             |        <> |        <> |        < |
    | vector_length | 8.794e-11 | 3.025e-11 |  0.00555 |
