...
* TODO Corrections                                                 :noexport:
** Envisioned approach 
   Redundancy in the list explaining the process: item 1 and 2 \to
   should be merge
* Plan                                                             :noexport:
** Introduction [3/3]
...
** My contribution
   ...
   1. Propose performance models of the computation kernel
   ...
   We tested this approach on a relatively simple kernel that computes the
...
** Structure  of the report
   ...
   encountered difficulties and how we dealt with it.
* Context
** HPC architectures
  ...
  *the cores* available and keep them busy even when I/Os occur to have the
  ...
  *Pipelining* is another kind of parallelism in which the processing of
  ...
  *GPUs* because for computation that can be well parallelized which
  ...
  CPUs and need to be programmed in a quite different manner and the
  ...
  chalenging and porting applications accross platforms is also
  complex and highly time consuming as the optimization are low level
  ...
  (see Figure\ref{fig:correlation}). As developers do not want to spend months to port the
...
*** Source-to-source transformation
    ...
    #+BEGIN_LaTeX
    \begin{lstlisting}[caption={Example of source-to-source code transformation\cite{Bondhugula:2008:PAP:1379022.1375595} which performs a loop fusion optimization. This reduces the loop overhead as it remains 2 loops instead of 4. It also improves the data locality with the variable A being re-used within a shorter period of time.}, label={lst:source-to-source1}]
    // Original code
    for ( i=0; i<N; i++)
      for ( j=0; j<N; j++)
...
** Problem analysis
  In a word, optimizing and porting HPC applications is tricky but
  tools exist to assist the developer in this 
  ...
  testing all the combinations (i.e, $2^{500} \approx 10^{50}$ combinations) to find
  ...
  because they cause the compilation to fail or the program to
...
*** Machine learning
    This technique is generally used to identify categories of programs
   ...
   for each categories of programs. Thus, it is an attempt to
   generalize and to reuse the knowledge from previous experiences. 
   ...
   program variants on a platform to estimate their performances on other
   ...
   As the efficiency of a search strategy is dependent on the structure of
   the search space, machine learning can be used to learn which search
   ...
   enough. To mitigate this problem, some, such as the framework
...
**** 
   ...
   \caption{\label{fig:function_charac}\textbf{Function characteristics}: 
    Figure\ref{fig:convex_function} shows the example of a convex 
    function. On this kind of functions, techniques such as gradient 
    search converge very quickly to the global optimal.
    Figure\ref{fig:non_convex_function} shows an example of 
    non-convexe function. This kind of function has many local optimum in which
    local search techniques can be stuck. Thus, global search and randomized techniques 
    are necessary to escape from them.
    Figure\ref{fig:non_smooth_function} shows the example of a function
    where at the derivative cannot be computed everywhere. Hence, derivative-based 
    free methods are required.
    Figure\ref{fig:auto_tuning_function} shows what the objective function 
    in auto-tuning looks like. It combines non-convexity, non-smoothness, discontinuities and noise}
   \end{figure}
   #+END_LaTeX
   ...
   local search algorithm would be stuck in a local optimum. The problem is that
   local optimum can be far from the global optimum. That is why, to
   escape from these local optimum, global search and randomization are more suited
   ...
   inefficient (see Figure\ref{fig:non_smooth_function}), the
   alternative is to use derivative-free based searches such as Nelder
   Mead Simplex. In auto-tuning the objective function is empirical and combines
   non-convexity, non-smoothness, discontinuities and noise(see
   Figure\ref{fig:auto_tuning_function}). Hence, it can be necessary 
   to build a surrogate. This is useful to remove the noise and as a
   result it facilitates the search. 
   ...
   much the hypothesis about the search space are wrong and sometimes
   it is difficult to know how it looks. For this reason, some have worked
   ...
   direct-search\cite{Audet04meshadaptive}, which is an extention of the
   ...
   Some people developed frameworks to characterize the search
   ...
   result can be further improved, and he has no clue about the quality
   of the solution. Our goal is to give more feedback and control,
   through a semi-automatic and interactive approach, to
   the user to guide him in the tuning of his application. Our
...
*** 
   ...
   the points are not well distributed, there are part of the space where
   there are lots of points and some other where there are just few. The
   ...
   \caption{\label{fig:DoE_examples} \textbf{Sampling techniques}: These figures shows the space coverage 
    of random sampling, LHS and D-Optimal sampling. The random sampling is the 
...
** Reproducible research
  Such experimental process mandates rigorous methodology.
...
** Case study
   ...
   which is a kernel that computes the Laplacian of an image and for
   which we want to minimize the time to compute a pixel. 
   Figure\ref{fig:boast_code} shows the BOAST code to generate a
   Laplacian kernel. Figure\ref{fig:kernel_code} shows one such
   generated kernel. This BOAST code was already implemented by the
   BOAST developers but as we will see finding the right parameter values
   was quite a challenge. There are multiple optimization that can be done to enhance
   the performance of this kernel each one of them are expressed by a
   parameter and its possible values. The parameters and their values we
   used to tune this applications are the following: 
   ...
   2. /Load_overlap/ is related to the vectorization. As vectors
      ...
      reduce the number of loads. As it is a factor with only 2
   ...
   5. /Y_component_number/ is used to specify how the work for a thread is
      ...
      thus improve the data usage. The impact is suspected to have an
   ...
   7. /Local_work_size_y/ (/lws_y/) determines how the threads are
      organized in a block and represents the number of threads in the
   ...
   The model we will consider are sums with at most level 2
   interactions of the previous one.
   ...
   reject all the combinations that would produce a too big kernel or that is not
...
** Controlling measurement
   ...
   #+begin_src R :results output graphics :file img/warm_up.pdf :exports none :width 9 :height 9 :session
     library(plyr)
     library(ggplot2)
   ...
   features such as power saving, frequency scaling, etc... Thus it
   ...
   compute a pixel, is sensitive to this. Thus we have to be protected
   ...
   switches to the performance mode. Thus the device does not provides
   all its capabilities immediately, hence the warm-up effect.

   The measurement process is made as follow:
   ...
   this effect is present. We also tried to quantify it along
   with the variability we could have between the different runs of
   the same version of the code in order to protect against
   it. Figure\ref{fig:warmup}, illustrates what we expected, there is
   a power saving mechanism on Nvidia K40 which turns the GPU into idle mode
   ...
   GPU does not have the time to switch to idle mode. So to be
   protected against the warm-up effect we only need to make at least four
   ...
   \caption{\label{fig:warmup}\textbf{Warm-up effect}: 
     ...
     for 5 different image sizes and for different vector lengths. We can see
...
* Envisioned general approach
   ...
   valuable information to the user to guide him and exploit his
   ...
   approach consists in sampling the search space with only few points to
   ...
   1. The search space is sampled by taking into account the objective
   ...
   2. Using linear regression, a model is built based on the
      hypothesis about the kernel performances
      provided by the user. This step allows the developer to test
      multiple hypothesis, by trying different models and keeping the
      most suited. This allows to determine what are the parameters
      ...
      order parameters are fixed. It also allows to estimate what kind
      of impact they have by finding their coefficients. If the user
      is not satisfied about the result of the linear regression he
      can ask additional points.
   3. Once the user has found a satisfying model, this latter is
      instantiated in order to predict the best values only for the
      relevant parameters.
   4. The search space is pruned by the fixing the relevant parameters
      to the estimated values.
   5. The remaining parameters are re-injected in the tuning process
      which iterates until all the wanted parameters are fixed.
   6. At the end the user has either the best estimated solution or at
      least the most interesting region of the search space.
   ...
   \begin{figure}[tbh]
   \centering
   \includegraphics[width=.8\linewidth]{img/process2.pdf}
   \caption{\label{fig:process}Semi-automatic approach workflow. This Figure shows 
            how linear regression techniques are combined with design of experiments techniques.}
   \end{figure}
   #+END_LaTeX
* Results
***                                                                :noexport:
    ...
    #+begin_src R :results output graphics :file ./img/results_hist.pdf :exports none :width 7 :height 8 :session
...
*** 
    ...
    search methods that are generally used in auto-tuning
    literature using an exploration budget of 120 points (0.5% of the
    search space) that must not be exceeded:
    - *RS*: is the uniform random search that takes points randomly in the
      search space with equal probabilities. 
    - *LHS*: it is a not a method used to search, it is a sampling
      technique which takes points randomly but which also maximizes the
      distance between them to cover the full search
      space. However we wanted to see how a search method based on it, would
      perform. 
    - *GS* (Greedy Search): we implemented a greedy search which
      ...
      optimum. In such case, the algorithm may not have used its whole
    ...
    - *GA*: BOAST embeds an implementation of a genetic algorithm. We
      ...
      different configurations tested, it was the one that gave the
      best results but it may be possible to obtain better results by
      tuning further the genetic algorithm parameters.  
    
    Although the linear regression of the mean is the classical tool to use in
    ...
    - *LM*: this corresponds to the iterative approach described in the
      previous section and relying on a classical linear model using least square
    ...
    - *RQ*: this approach is similar to the previous one but relies on
      ...
      smallest values depend on the various input parameters. 
    ...
    made by trying the full model and eliminating useless parameters
    ...
    8. Setting the factor /threads_number/ and pruning the
       ...
       #+END_LaTeX
    9. At this step, after pruning, only 4 points remain in the search space.
       As we are below (119 points) the maximum number of points (120 points),
       we take the best remaining combination of these 4 points.  
 ...
 #+CAPTION: This table shows the minimum, first quartile, median, mean, third quartile and maximum slowdown including the mean number of points used by each method.
    ...
    maximum slowdown that is higher than 38.8%. A likely explanation of the efficiency
    ...
    search. The GS strategy is extremely inefficient since almost  half of
    ...
    LM and RQ very rarely gives worse solutions than GA or RS.
   ...
   \caption{\label{fig:search_comparison}Comparison of the efficiency of the different search
      methods. The histogram of the slowdown (1000 values for
      ...
      green dotted line depicts the sample median. The arrows show the maximum values, expect for GS which is out of the range.}
   \end{figure}
   #+END_LaTeX
...
*** 
   ...
   \end{minipage}
   \caption{\label{fig:combination_distribution}These figures present the slowdown of the combination 
  ...
  Figure\ref{fig:combination_distribution} and Table\ref{tab:search-space-characteristics}
  ...
  LHS obtains results good. Among the 23120 combinations there are 170 which
  ...
  | 6        | 6           | 1      | 2         | true    | 512     | 8     | 1.015301 |
   ...
   the same everywhere, hence our data are
   heteroscedastics. This is because the noise does not 
...
*** 
   ...
   case and it biases the standard errors and thus the coefficient of
   ...
   noise, the evolution of the minimum value does not follow the
   ...
   \caption{\label{fig:qr-example}\textbf{Linear regression vs quantile regression}: 
    These figures show the impact of different factors on the time to compute a pixel and 
    the least square regression line in red and a quantile regression line of the 5th percentile in blue .
   ...
   do not know anything about it. In the Figure\ref{fig:lm_x_comp}, the
   evolution of the mean and the evolution of the minimum are not
...
*** 
  ...
  #+CAPTION: This table presents the fit of an incorrect model but which make a correct prediction of low values for the time to compute a pixel
  #+NAME:   tab:lucky-fit
    |               |      Coef | Std. err. | p-values |
    |---------------+-----------+-----------+----------|
    | /             |        <> |        <> |        < |
...
* Technical difficulties
  ...
  raised however several technical difficulties which we have not
...
**** 
    ...
    too big kernel simply does not compile. 
   ...
   \end{minipage}
   \caption{\textbf{Search space constraints}: These figures represent the example of some of the 
   ...
   Figure\ref{fig:search_space_3d_constrained_ugly} shows the same function but only with the feasible points. 
   Figure\ref{fig:search_space_3d_constrained} shows that we can ignore some constraints and just round 
   the values. 
   Figure\ref{fig:search_space_3d_barrier} illustrate the barrier approach we could use to deal with 
...
**** Screening design 
    ...
    as it allows to detect which parameters have the biggest
...
**** LHS
   ...
   \end{minipage}
   \caption{\textbf{Space coverage}: These figures show the space coverage of 
    the LHS and random sampling with our search space. With our constrained search space, LHS 
    loses it property of maximum distance between points and do not have a better 
...
** Quantile regression
   ...
   circumvent them. Thus we had to implement another method to
...
* Conclusion
  ...
  3. We also demonstrated the limitations of the
     ...
     no certainty about the error law and the estimation of the mean
  ...
  regression is more suited than the classical least square regression when
  the noise is non-uniform, but if we cannot assess if a model is
  accurate not, we are stuck. Thus it is crucial to improve this
  point.  
...
* Acknowledgments
I would like to thank my supervisors for allowing me to make my
internship in their teams and for this subject I really
enjoyed. I thank them for their availability and their help and thanks
