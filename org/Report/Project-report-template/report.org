...
* Envisioned general approach
   ...
   process where the user refines is model according to the extracted
   information.
...
*** 
    ...
    purpose we automated the process. For this we decided to apply
    exactly the same strategy each time without considering the
    random set:
    1. Sampling 50 points at random
    2. Setting the factors vector length and lws y and pruning the
   ...
   #+END_LaTeX
    ...
    | GR     | 1.000 |   1.348 |  1.799 |   6.314 | 124.800 | 6.460 |    22.171 |
    | GRM    | 1.000 |   1.072 |  1.187 |   1.333 |   3.164 | 1.227 |       120 |
...
*** 
  ...
  | 6        | 6           | 1      | 2         | true    | 512     | 8     | 1.015301 |
   ...
   \end{minipage}
   \caption{\label{fig:heteroscedasticity}\textbf{Non-uniform variance}: This figures shows in our data the noise 
    does not the same law.}
   \end{figure}
   #+END_LaTeX   
...
*** 
   ...
   \caption{\label{fig:qr-example}\textbf{Linear regression vs quantile regression}: 
    These figures shows the impact of different factors on the time to compute a pixel.
    On Figure\ref{\label{fig:lm_v_len.pdf}} The variance ins not the same every 
    where the the minimum follow the same tendency as the mean.
    On Figure\ref{fig:lm_x_comp}The variance is not uniform as in the previous case and the minimum
    does not follow the same tendency as the mean. Hence the classical linear regression
    fails to predict the evolution of the minimum when the error 
    law is not uniform while quantile is more suited to this (see Figure\ref{fig:rq_v_len} 
    and Figure\ref{fig:rq_x_comp}).}
   \end{figure}
   #+END_LaTeX
...
*** 
    ...
    process (see Table\ref{tab:worst-solution}). Indeed, vector length and lws y 
  ...
  #+NAME:   tab:correct-fit
   ...
   of these cases when fitting the model vector length + lws y has a
   poor R-squared of 0.1665 (see Table\ref{tab:lucky-fit}). Only lws y
   is significant. The standard error of vector length is high,
  ...
  #+NAME:   tab:lucky-fit
   ...
   \end{minipage}
   \caption{\textbf{Comparison between predicted and observed values}: A correct model has a correlation
   between predicted and observed values. Even if the model is not perfect the most important is the 
   correlation between low values. Which means the it correctly estimate good performing combinations 
   (see Figure\ref{fig:good_prediction} and Figure\ref{fig:lucky_prediction}).}
   \end{figure}
   #+END_LaTeX
...
* Technical difficulties
  ...
  search space which not a simple compact and convex $k$-cell but
...
**** 
   ...
   \caption{\label{fig:search_space_3d}Model of the objective function}
   \end{minipage}
   \begin{minipage}{.45\linewidth}
   ...
   \end{minipage}
   \caption{\textbf{Search space constraints}: These figure represents the example of some of the 
    constraints on the paramters elements number and y component number and the envisioned approach
   to cope with it.}
   \end{figure}
   #+END_LaTeX
...
**** LHS
   ...
   \end{minipage}
   \caption{\textbf{Space coverage}: With our constrained search space, LHS 
...
** Quantile regression
   ...
   building on the presentation of Charpentier\cite{FIXME}, we used a
...
* Conclusion
  ...
  The main contribution of this report is to design an innovative
  ...
  method performed very efficiently and significantly outperformed the
  other. The few situations where it returned a poor combination of
  parameters come from the fact that for evaluation purpose we
  automated the process by using fix decisions without considering the
  working sample. We explained how in such situation, when the analyst
  is part of the the optimization loop, he is able to consider all the
  ...
  correct decisions. We also demonstrated the limitations of the
  ...
  additional work compute correct coefficients of determination,
  ...
  regression. We also learned that some good idea from design of
  ...
  Finally, we want to stress that for such an experimental study, it
  ...
  We managed to get good results on a Laplacian kernel but it remains
  a lot of work to do. In the future we would like to apply our approach
  to more complicated cases such as a matrix multiply and on
  different architectures to see if it can performs as well.

  Also, among the previous raised technical difficulties, one of the
  major axis of improvement concerns the quantile regression. It is
  important to be able to compute confidence interval, standard error,
  coefficients of determination to be able to assess the quality of a
