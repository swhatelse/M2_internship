...
** The reason why linear regression is not suited
   #+begin_src R :results output graphics :file (org-babel-temp-file "figure" ".png") :exports results :width 800 :height 400 :session :caption \MyCaption{Non-uniform noise}
     library(ggplot2)
     library(plyr)
     library(gridExtra)
     ...
     p1 <- qplot(factor(df$vector_length), df$time_per_pixel) + 
         geom_point(alpha=0.1) + 
         geom_hline(yintercept=min(df$time_per_pixel), color="red", linetype=2) +
         geom_errorbar(data=err_v_len,aes(x=factor(vector_length),y=mean, ymin=mean-err, ymax=mean+err)) +
         ggtitle("Impact of elements number") +
         theme(axis.text=element_text(size=12),
               axis.title=element_text(size=14,face="bold"))

     p2 <- qplot(factor(df$x_component_number), df$time_per_pixel) + 
         geom_point(alpha=0.1) + 
         geom_hline(yintercept=min(df$time_per_pixel), color="red", linetype=2) +
         geom_errorbar(data=err_x_comp,aes(x=factor(x_component_number),y=mean, ymin=mean-err, ymax=mean+err)) +
   ...
   #+RESULTS:
   [[file:/tmp/babel-3694SAZ/figure369453Z.png]]
   
   Linear regression has already been used successfully for auto-tuning problems 
