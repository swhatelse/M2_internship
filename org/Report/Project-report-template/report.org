    ...
    #+begin_src R :results output graphics :file ./img/results_hist.pdf :exports none :width 6 :height 8 :session
      df_all_methods <- read.csv("../../../data/2016_04_08/pilipili2/18_08_24/all_search_3.csv", strip.white=T, header=T)  
        ...
        ggplot(df_all_methods ) + 
            ...
            geom_rect(data = df_err, aes(xmin=mean-err, xmax=mean+err, ymin=0, ymax=100, fill="red"), alpha=0.3) +
...
*** 
    ...
    which is less than 7.3% half of the time, a mean slowdown of 9.8%
    and a maximum slowdown of 39%. 
    The technique using least square regression is extremely efficient,
    most of the time it converges to the same solution 99% of the time
    which has a slowdown of 1.2%. However it never find the best
    solution.With a mean slowdown of 1.4% it is 6.7 times better than
    GA. It only failed one time giving a solutions with a slowdown of
    21.4% which is still better than worst solution of the GA.
    ...
    slowdown of 2.2% which almost 10 times better than LM and better
    than the mean of GA. However like LM it does not find the optimal
    solution.
    ...
    we could not improve the best solution return by LM.
   ...
   #+END_LaTeX
    ...
    | GRM     | 1.000 |  2.153 | 1.233 |  1.208 |     25.71 |
    | LHS     | 1.009 |  1.388 | 1.155 |  1.172 |      99.6 |
    | RS      | 1.011 |  1.341 | 1.098 |  1.079 |       120 |
    | GA      |  1.00 |  1.390 | 1.098 |  1.073 |       120 |
    | LM      | 1.012 |  1.214 | 1.014 |  1.012 |       119 |
    | RQ      | 1.012 |  1.022 | 1.012 |  1.012 |       119 |
...
* Analysis
  ...
  automatized our approach by blindly using the same model without
  considering the working set of points and by using exactly the same
  strategy. This gives us an overview of how it could perform but it
  is not intended to use like this. Hence this part explains why some
  time we never get the same solution and why we do not get the best
  optimal solution.
...
**** Best combinations
     #+begin_src R :results output :session :exports both
       ...
       head(sorted[,!names(sorted) %in% c("vector_recompute")],20)
       options(width=w)
     #+end_src

     #+RESULTS:
     #+begin_example
           elements_number y_component_number vector_length temporary_size load_overlap threads_number lws_y time_per_pixel slowdown
     13752               6                  6             1              2         true           1024     2   1.165013e-10 1.000000
     9843                6                  6             1              2        false            256     4   1.171493e-10 1.005562
     17930               5                  5             1              2         true            256     4   1.173798e-10 1.007540
     10660               5                  5             1              2        false            256     4   1.175615e-10 1.009100
     19494               6                  6             1              2        false           1024     8   1.176425e-10 1.009796
     14258               6                  6             1              2        false            128     4   1.177325e-10 1.010568
     12261               6                  6             1              2         true           1024     4   1.178422e-10 1.011509
     22098               5                  5             1              2        false            512     4   1.178655e-10 1.011710
     17258               6                  6             1              2         true            256     1   1.179200e-10 1.012177
     6468                6                  6             1              2        false            128     2   1.179765e-10 1.012663
     11332               6                  6             1              2         true            128     2   1.180805e-10 1.013555
     21903               6                  6             1              2         true            256     2   1.180832e-10 1.013578
     12450               5                  5             1              2         true            512     4   1.180929e-10 1.013662
     16041               6                  6             1              4        false           1024     4   1.180968e-10 1.013695
     8830                6                  6             1              4         true            512     8   1.181440e-10 1.014100
     12719               6                  6             1              4         true           1024     4   1.181529e-10 1.014177
     19283               6                  6             1              2        false            256     1   1.182020e-10 1.014598
     9305                6                  6             1              2         true           1024     8   1.182351e-10 1.014882
     16588               6                  6             1              2         true            128     1   1.182590e-10 1.015087
     17018               6                  6             1              2         true            512     8   1.182839e-10 1.015301
#+end_example
...
** TODO Sampling strategy                                          :noexport:
