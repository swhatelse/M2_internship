...
*** 
    ...
    - GA: BOAST embeds an implementation of a genetic algorithm.
    ...
    There are many ways of performing linear regression. For this
    reason, with our approach, we evaluated two techniques:
    ...
    search space. Figure\ref{search-comparison} shows the results.
   #+BEGIN_LaTeX
   \begin{figure}[]
   \centering
   \includegraphics[height=.15\textheight]{./img/results_hist.pdf}
   \caption{\label{fig:search-comparison}Search comparison}
   \end{figure}
   #+END_LaTeX
* Analysis
