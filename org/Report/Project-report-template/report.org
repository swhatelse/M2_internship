...
them with care and write specialized code. Tools exists to help 
...
and the exhaustive search is prohibitive. Hence auto-tuning consider this as 
...
user from the optimization process and it is difficult to know if further 
improvement can be done and evaluate the quality of the solution.
We propose a semi-automatic approach that gives power back to the user. 
This approach is based on regression techniques to predict the objective 
function combined to techniques inspired from design of experiments which aim 
at reducing the number of experiments. 
We evaluated our approach on the case of a Laplacian kernel and compared it to 
\end{abstract}
\abstractintoc
...
** Code generation and opportunities  
  ...
  etc... Many other optimization opportunities exist but it is to
...
** My contribution
   ...
   hypothesized in order to guide the tuning process combining
   techniques of design of experiments.  
   ...
   used in the auto-tuning literature such as random search, genetic
   ...
   characteristics of our search space we find out the simple
   techniques are the most suited.

** Structure  of the report
   The second part of this report exposes context of this work and the
   auto-tuning problem. The third part makes a summary of the
   different techniques used in auto-tuning and design of experiments. 
   problem. The fourth part exposes how this work was made, gives
   details about the case study and how measurements were done. 
   The fifth part explains our semi-automatic approach. 
   The sixth part presents the comparison between our approach and
   other search techniques and the seventh provides the analysis. The
   eighth part exposes the encounter difficulties and how we dealt
   with it.
* Context
** HPC architectures
...
** Case study
   ...
   #+BEGIN_LaTeX
   mp021 = vload4(0, &psrc[x + 0 + 3 + (w) * (y + 0)]);
...
*** 
  ...
  local optimum is this area because we try to run the GS and the
...
* Conclusion
  ...
  2. We compared our approach to other optimization techniques used
     ...
     process by using fix decisions without considering the working
  ...
  5. We want to stress that for such an experimental study, it
