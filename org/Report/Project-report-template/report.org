...
** TODO Model choice, refinement and pruning decision
   Usually the expert understand every optimization of its kernel and
...
***                                                                :noexport:
    ...
    #+begin_src R :results output graphics :file img/search_space_3D.pdf :exports both :width 12 :height 6 :session                                                                   
      ...
      persp3D(xlab="elements_number", ylab="y_component_number", zlab="time per pixel in s", z=combinations, phi=20, theta=220, clab=c("time per pixel in s"),colkey = list(side=1,length = 0.5, width=0.4))
    #+end_src
    
    #+RESULTS:
    [[file:img/search_space_3D.pdf]]
    
    #+begin_src R :results output graphics :file img/search_space_3D_constrained.pdf :exports both :width 8 :height 6 :session
      g <- function(x){
          ifelse(x[1] <= x[2] * 4, f(x), 0)
      }
    ...
    #+begin_src R :results output graphics :file img/search_space_3D_barrier.pdf :exports both :width 8 :height 6 :session
      h <- function(x){
      ...
      persp3D(xlab="elements_number", ylab="y_component_number", zlab="time per pixel in s", z=combinations, phi=20, theta=190, clab=c("time per pixel in s"),colkey = list(side=1,length = 0.5, width=0.4))
