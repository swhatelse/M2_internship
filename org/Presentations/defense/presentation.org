...
\small
#+END_LaTeX
 - Scientific applications \to increasing need of computational power
- Supercomputers become bigger and bigger but more complex
...
- And *porting even more* \to developers cannot afford to spend month in
  porting his application on a new platform

#+BEGIN_LaTeX
\normalsize
#+END_LaTeX
*** Optimization of scientific applications
- Computation intensive \to *focus on kernels* optimization
- Doing it by hand \to neither scalable nor portable
- Compiler not enough
- Need code generation tools: 
  - Allow the developer to write portable code by only *describing
    optimization* 
...
:BEAMER_act: <4>
:END:
- Efficient of a method depends the characteristics of the search space
- Fully automatic approach \to black box and *user excluded*
- Does not make sense, the application is *not a black box* for the user \to
  he knows the kernel and thus have a important role in the tuning process.
...
- Based on:
  - Design of experiments \to investigate the search at a minimal
    experimental cost
  - Linear regression (least square and quantile regression)
...
2. Apply linear regression:
   - To find relevant parameters and *remove useless ones* 
