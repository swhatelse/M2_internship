...
** Overview
   ...
   problem. We can distinguish three main category of approach. The
   first one does not rely on any kind of information about the
   problem like random search or genetic algorithms works. The second
   one use partial information  or "on the fly" about the problem like
   direct search methods. And the last one use information build on
   top of models.  
** Problem
